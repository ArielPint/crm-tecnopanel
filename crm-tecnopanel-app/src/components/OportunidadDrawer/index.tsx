  ...
  function renderEtapaForm() {
    ...
    if (e === 'Ingeniería') return (
      <div className="space-y-3">
        ...
        {ta('descripcion_estructura','Descripcion de estructura','Tipo de estructura...')}
        <div className="grid grid-cols-3 gap-3">{field('carga_viento','Viento (km/h)','number','0')}{field('carga_nieve','Nieve (kg/m²)','number','0')}{field('zona_sismica','Zona sismica','text','Z1...')}</div>
        {ta('observaciones_tecnicas','Observaciones tecnicas','')}
      </div>
    )
    if (e === 'Cubicación') return <div className="space-y-3">{field('acero_kg','Acero estructural (kg)','number','0')}{field('paneles_m2','Paneles (m²)','number','0')}{field('cubierta_m2','Cubierta (m²)','number','0')}{field('pilares_und','Pilares (und)','number','0')}{ta('lista_materiales','Materiales adicionales','')} {ta('observaciones','Observaciones','')}</div>
    if (e === 'Presupuestos') return <div className="space-y-3">{field('costo_materiales','Costo materiales (CLP)','number','0')}{field('costo_mano_obra','Mano de obra (CLP)','number','0')}{field('costo_transporte','Transporte (CLP)','number','0')}{field('margen_porcentaje','Margen (%)','number','0')}{field('precio_final','Precio final (CLP)','number','0')}{ta('notas_presupuesto','Notas','')}</div>
    if (e === 'Revisión Vendedor') return <div className="space-y-3">{field('descuento_porcentaje','Descuento (%)','number','0')}{field('plazo_entrega_dias','Plazo entrega (dias)','number','0')}{ta('condiciones_comerciales','Condiciones comerciales','')}{ta('notas_revision','Notas del vendedor','')}</div>
    if (e === 'Revisión Cliente') return <div className="space-y-3">{ta('feedback_cliente','Feedback del cliente','')}{field('modificaciones_solicitadas','Modificaciones','text','')}{ta('acuerdos','Acuerdos alcanzados','')}</div>
    if (e === 'Evaluación Crediticia') return <div className="space-y-3">{field('limite_credito','Limite de credito (CLP)','number','0')}{field('plazo_pago_dias','Plazo de pago (dias)','number','0')}{field('resultado','Resultado','text','Aprobado / Rechazado')}{ta('condiciones_credito','Condiciones','')}{ta('observaciones_finanzas','Observaciones','')}</div>
              ...
              : historial.map((h, i) => (
                <div key={h.id} className="flex gap-3">
                  ...
                  <div className="pb-4 flex-1">
                    ...
                    {h.usuario && <p className="text-xs text-gray-400 flex items-center gap-1 mt-0.5"><User size={10} />{(h.usuario as unknown as Profile).nombre} {(h.usuario as unknown as Profile).apellido}</p>}
                  </div>
                </div>
              ))}
            </div>
